...
bool CAnimatedMeshSceneNode::setAnimation(const c8* name, bool playAnimationLooped)
{
	bool result=false;
	s32 index = Mesh->getAnimationIndex(name);
	if(index < 0)
		return false;
	
	io::IAttributes* attr;
	if(Mesh->getAnimation(index, &attr))
	{
		setFrameLoop(attr->getAttributeAsInt("startFrame"), 
			attr->getAttributeAsInt("endFrame"));		
		setLoopMode(playAnimationLooped);		
		result = true;
	}
	attr->drop();
	return result;
