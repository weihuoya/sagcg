...
namespace irr
{
	...
	private:
		...
		public:
			...
			CCursorControl(CIrrDeviceIPhone* dev, const SIrrlichtCreationParameters& params)
            : Device(dev), IsVisible(true)
			{
               screenWidthHeight = params.WindowSize;
               CursorPos=core::position2d<s32>(1.0 , 1.0);
               CursorPosOld=core::position2d<s32>(1.0 , 1.0);
			}
			...
			{
				updateCursorPos();
				return core::position2d<f32>(CursorPos.X / (f32)screenWidthHeight.X,
                                             CursorPos.Y / (f32)screenWidthHeight.Y);
			}
		...
		private:
			...
			void updateCursorPos()
			{
				...
				CursorPos.X = getIphoneCursorX();
				CursorPos.Y = getIphoneCursorY();
                
				if (CursorPos.X < 0)
					CursorPos.X = 0;
				if (CursorPos.X > (s32)screenWidthHeight.X)
					CursorPos.X = screenWidthHeight.X;
				if (CursorPos.Y < 0)
					CursorPos.Y = 0;
				if (CursorPos.Y > (s32)screenWidthHeight.Y)
					CursorPos.Y = screenWidthHeight.Y;
                
			}
            
			CIrrDeviceIPhone* Device;
			core::position2d<s32> CursorPos;
            core::position2d<s32> CursorPosOld;
            core::position2d<u32> screenWidthHeight;
			bool IsVisible;
		};
