...
// How to use: device->getSceneManager()->addExternalMeshLoader(new scene::CFBXMeshFileLoader(device->getSceneManager()));
	...
	namespace scene
	{
		...
		namespace
		{
			...
			void FBXRootTokenType::ProcessToken ( core::stringc &Token, FBXGrammarConsumer *reader )
			{
				...
				{
					///here are the keyframes/curves
					printf ( "parsing Takes:\n" );
					reader->readOpenBrace();
					core::stringc type ( "OO" );

					while ( reader->ConsumeNextToken ( Token ) != "}" )
					{
						//type=reader->ConsumeNextToken();//>ConsumeString();
					}
				}
				else if ( Token == "Version5:" )
				{
					printf ( "parsing Version5:\n" );
					reader->readOpenBrace();
					core::stringc type ( "OO" );
