...
io::path CFileSystem::getAbsolutePath(const io::path& filename) const
{
	...
	#endif
	return tmp;
#elif (defined(_IRR_POSIX_API_) || defined(_IRR_OSX_PLATFORM_)) && !defined(__native_client__)
	c8* p=0;
	c8 fpath[4096];
	fpath[0]=0;
	...
	if (FileSystemType == FILESYSTEM_NATIVE)
	{
		...
		if (dirHandle)
		{
			...
			while ((dirEntry=readdir(dirHandle)))
			{
				...
				#if !defined(_IRR_SOLARIS_PLATFORM_) && !defined(__CYGWIN__) && !defined(__native_client__)
